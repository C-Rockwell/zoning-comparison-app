...
// Helper to get (and lazily create) the dimension presets directory
async function getPresetsDir(req) {
  const projectsDir = await getProjectsDir(req)
  const presetsDir = join(projectsDir, '_dimension-presets')
...
// GET / - List all dimension presets
...
// POST / - Save a dimension preset
router.post('/', async (req, res) => {
  try {
    const presetsDir = await getPresetsDir(req)

    const { name, ...dimensionStyleData } = req.body
...
// GET /:name - Load a specific dimension preset
router.get('/:name', async (req, res) => {
  try {
    ...
    if (!existsSync(filePath)) {
      return res.status(404).json({ error: 'Dimension preset not found' })
    }
...
// DELETE /:name - Delete a dimension preset
router.delete('/:name', async (req, res) => {
  try {
    ...
    res.json({ success: true, message: 'Dimension preset deleted' })
  } catch (err) {
    res.status(500).json({ error: err.message })
  }
})
